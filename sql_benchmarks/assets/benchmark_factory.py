...
def make_benchmark_asset(name, engine, used_tables, raw_template, static_meta, extra_context=None):
    ...
    if engine in ("postgres", "quack", "quack_pushdown", "quack_adbc", "quack_arrow", "postgres_transport"):
        # postgres_transport shares the one Postgres container (restarted for
        # cold cache), so it must run single-lane like the other exclusive engines.
