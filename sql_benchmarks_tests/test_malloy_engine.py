...
@pytest.fixture
def client(tmp_path):
    return MalloyClient(package_dir=str(tmp_path / "pkg"), port=4001,
...
def test_bulk_load_is_idempotent_and_appends_tables(client, parquet):
    ...
    model = open(os.path.join(client.package_dir, "bench_small.malloy")).read()
...
def test_partitions_get_separate_models(client, parquet):
    ...
    assert os.path.exists(os.path.join(client.package_dir, "bench_small.malloy"))
    assert os.path.exists(os.path.join(client.package_dir, "bench_large.malloy"))
